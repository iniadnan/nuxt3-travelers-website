<script setup>
defineProps(["title", "image", "price", "location", "star"]);
</script>

<template>
  <article class="swiper-slide w-[85%] md:w-auto cursor-pointer group">
    <!-- HEADER -->
    <div class="relative mb-[26px] w-full rounded-[20px] overflow-hidden">
      <img
        class="h-[380px] md:h-[420px] lg:h-[475px] w-full group-hover:scale-110 transition duration-300 ease-in-out object-cover rounded-[20px]"
        height="475"
        width="300"
        :src="image"
        alt="Bali"
      ...
      <!-- BOX LABEL -->
      <div
        class="absolute right-4 bottom-4 flex items-center justify-end gap-x-[13px] z-10"
      >
        <div
          class="bg-white rounded-[15px] py-[12px] md:py-[14px] lg:py-[18px] px-[14px] md:px-[15px] flex items-center gap-x-2"
        >
          <svg
            width="24"
            height="24"
          ...
          <span
            class="font-medium text-[16px] md:text-[18px] lg:text-[20px] text-[#1E1E1E]"
            >{{ location }}</span
          >
        </div>
        <div
    ...
    <div class="flex items-center justify-between">
      <h3 class="text-2xl text-[#1E1E1E]">{{ title }}</h3>
      <p class="font-bold text-2xl text-[#1E1E1E]">${{ price }}</p>
    </div>
  </article>
</template>
